...
static void DrawFreeSpaceCellEdges(uint32_t P_seg_idx, uint32_t Q_seg_idx, FD_freespace_edge_data edge_data, Rectangle base)
{
	...
	{ // if entry or exit from that edge is possible
		if (edge_data.reachable[offsets[i]])
